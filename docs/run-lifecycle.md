...
- the workspace root is inferred from the spec location
- if the spec lives under `specs/` or `examples/`, the parent directory is treated as the workspace root
- the default `config/factory.config.json` is resolved from that workspace root
- the persisted `workspacePath` in `run-state.json` is the inferred workspace root, not the caller shell directory
...
- status changes are rejected if the target task's dependencies are not completed
- terminal tasks are not implicitly reopened through normal status updates
...
- `handoff` and `tick` resolve the default doctor report from the run workspace, not the caller cwd
